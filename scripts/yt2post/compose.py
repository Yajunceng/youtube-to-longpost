...
钩子开头、自然分段、适量 emoji、结尾带原链接 + 3-5 个话题标签、【不要 markdown 标记】。
"""
...
- 第一句是抓人的钩子,可带 1 个 emoji 提升点击欲。
- 自然分段(段落间空行)。段落内可用少量 emoji 做重点标记或视觉分隔,别堆砌(整篇控制在适量,服务可读性而非卖萌)。
- 结尾依次:① 原视频链接:{url} ② 另起一行放 3-5 个相关话题标签(形如 #海外手机卡 #eSIM,中英文均可,贴合视频主题、利于曝光)。
- 【不要】任何 markdown 标记:不要 # 标题、不要 **加粗**、不要 - 列表、不要 ` 代码符号。
  原因:X 不渲染 markdown,这些符号会原样显示成乱码。
  注意:emoji 和结尾的 #话题标签 是 X 原生语法、不是 markdown,允许使用。
