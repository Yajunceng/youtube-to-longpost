...
把口播逻辑和画面关键信息缝成一篇连贯长文,产出可直接发 X(推特)的格式,严格按 `AGENT_INSTRUCTIONS.md`:
- **风格是关键:叙事+干货平衡**。像懂行的人跟朋友讲一件事的来龙去脉,不要写成"背景→优点→工具清单→分步骤"的说明书。
  - 开头用有张力的钩子/场景切入(如"想长期保一个海外手机号,现在越来越难了"),不要"本视频介绍如何…"
  - 有"带你看懂这件事"的视角和判断,该下结论就下(哪种方案更值、坑在哪)
  - 数字编号分节(1、2、3…)推进叙事,把操作步骤融进叙事,别列"第一步第二步"清单
  - **关键干货必须保留**:具体数字、专有名词、易错点(如激活码前加大写 LPA、175 天保号、+44 去掉开头 0)
- 一整篇连贯长文,面向 X Premium(上限 25000 字),不是 thread、不是要点列表
- 自然分段(段落间空行),段内可用少量 emoji 标重点/分隔(适量,别堆砌)
- 结尾依次:原视频链接 → 另起一行 3-5 个相关话题标签(如 `#海外手机卡 #eSIM`)
- **不要任何 markdown 标记**(不要 # ## ** - ` 等)——X 不渲染 markdown,会显示成乱码;但 emoji、`#话题标签`、正文里的"1、2、"数字编号都允许
